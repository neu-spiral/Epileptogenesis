...
def load_data_df(processed_data_path,NBF=False):
    ...
    #dwi 
    y = all_features_df["Late Seizure Label"]
    # y = all_features_df["Late Seizure Label"].to_numpy()
    X_dwi = all_features_df[dwi_columns].to_numpy()
...
def impute_data_df(imputer,train_df,test_df=None,fmri_key=3,neighbors=1):
    ...
    # Impute data for test
    if test_df is not None:
        x_fmri=test_df.iloc[:,1+((fmri_key-1)*166):1+(fmri_key*166)]
        x_dmri=test_df.iloc[:,499:562]
        x_eeg=test_df.iloc[:,564:567]
        merged_df = x_fmri.join(x_dmri)
        test_df_f = merged_df.join(x_eeg)
